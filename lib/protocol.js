...
function makeAnnouncement({ room, venue, priority, targetLanguage, body, translatedBody }) {
  ...
  const translations = {}
  if (targetLanguage === 'es' && typeof translatedBody === 'string' && translatedBody.trim()) {
    translations.es = translatedBody.trim()
  }
...
function validateAnnouncement(value) {
  ...
  } else if (!isNonEmptyString(value.translations.es, MAX_BODY_LENGTH * 2)) {
    errors.push('translations.es must be present')
  }
...
function parseWireMessage(data) {
  ...
  if (!message || message.type !== 'announcement') throw new Error('Unsupported wire message')
  const result = validateAnnouncement(message.announcement)
  if (!result.ok) throw new Error(`Invalid announcement: ${result.errors.join(', ')}`)
  return message
}
